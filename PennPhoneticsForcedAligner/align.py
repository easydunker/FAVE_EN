...
sr_models = None # only set this if this module will be imported
...
def writeTextGridWithCn(outfile, word_alignments, sentenceCn) :
	# make the list of just phone alignments
	phons = []
	for wrd in word_alignments :
		phons.extend(wrd[1:]) # skip the word label
	...
	fw.write(str(len(wrds)) + '\n')
        for k in range(len(wrds) - 1):
                if (wrds[k][0] == 'sp'):
                    ...
                    fw.write(str(wrds[k][1]) + '\n')
                    fw.write(str(wrds[k+1][1]) + '\n')
                    if (idxInSentence > len(sentenceCn)-1):
                        fw.write('"' + wrds[k][0] + '"' + '\n')
                    else:
                        while(sentenceCn[idxInSentence] == ' '):
                            if (idxInSentence > len(sentenceCn) - 1):
                                break
                            else:
                                idxInSentence = idxInSentence + 1
                        if (idxInSentence > len(sentenceCn) - 1):
                            fw.write('"' + wrds[k][0] + '"' + '\n')
                        else:
                            fw.write('"' + sentenceCn[idxInSentence].encode('utf-8') + '"' + '\n')
                            idxInSentence = idxInSentence + 1
        fw.write(str(wrds[-1][1]) + '\n')
	fw.write(str(phons[-1][2]) + '\n')
	while(idxInSentence < len(sentenceCn) and sentenceCn[idxInSentence] == ' '):
            if (idxInSentence > len(sentenceCn) - 1):
                break
            else:
                idxInSentence = idxInSentence + 1
